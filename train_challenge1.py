...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("challenge1_training.log"), logging.StreamHandler()],
)
    ...
    def get_lr(self) -> List[float]:
        ...
        if self.last_epoch < self.warmup_epochs:
            ...
            return [
                ...
                + (self.base_lr - self.warmup_start_lr) * warmup_factor
...
class Challenge1Trainer:
    ...
    def __init__(
        self, config: Dict[str, Any], checkpoint_path: Optional[str] = None
    ) -> None:
        """
    ...
    def _validate_config(self) -> None:
        ...
        if self.gpus["cuda_available"]:
            logger.info(
                f"GPU Memory: {[f'{mem:.1f}GB' for mem in self.gpus['gpu_memory']]}"
            )
        logger.info("=" * 70)
    ...
    def _check_gpu_requirements(self):
        ...
        required_memory = self.config["hardware"].get(
            f"{self.encoder_type}_memory_gb", 8
        )

        for i, (gpu_name, gpu_memory) in enumerate(
            zip(self.gpus["gpu_names"], self.gpus["gpu_memory"])
        ):
            available_gb = int(gpu_memory)
            if available_gb < required_memory:
                logger.warning(
                    f"GPU {i} ({gpu_name}) has {available_gb}GB but {self.encoder_type.upper()} needs {required_memory}GB"
                )
                logger.warning("Consider reducing batch size or using CNN encoder")
            else:
                logger.info(
                    f"GPU {i} ({gpu_name}): {available_gb}GB (need {required_memory}GB)"
                )
    ...
    def setup_logging(self):
        ...
        experiment_name = (
            f"{self.config['logging']['experiment_name']}_{self.encoder_type}"
        )
        ...
        self.tb_logger = TensorBoardLogger(
            save_dir="logs", name=experiment_name, version=None
        )
    ...
    def create_model(self) -> Challenge1Model:
        ...
        if self.gpus["cuda_available"]:
            logger.info(
                f"   • GPU-Optimized Batch Size: {self.gpu_config['batch_size']}"
            )
    ...
    def create_callbacks(self) -> List[pl.Callback]:
        ...
        checkpoint_callback = ModelCheckpoint(
            ...
            filename=f"challenge1_{self.encoder_type}"
            + "-{epoch:02d}-{val/total_loss:.3f}",
            ...
            verbose=True,
            save_on_train_epoch_end=self.config["logging"][
                "save_on_train_epoch_end"
            ],  # Only save at validation end, not every epoch
        )
    ...
    def create_trainer(self, callbacks: List[pl.Callback]) -> pl.Trainer:
        ...
        if (
            ...
            and "transformer_config" in self.config.get("comparison", {})
        ):
            batch_size = self.config["comparison"]["transformer_config"]["batch_size"]
        elif self.encoder_type == "cnn" and "cnn_config" in self.config.get(
            "comparison", {}
        ):
        ...
        if self.config["model"]["encoder_type"] == "transformer":
            ...
            logger.info(
                "Disabling strict determinism for transformer encoder due to adaptive pooling operations"
            )
        ...
        if self.gpus["cuda_available"]:
            ...
            if self.config.get("debug", False):
                ...
                if self.gpus["num_gpus"] > 1:
                    ...
                    logger.info(
                        f"Using DDP strategy with {self.gpus['num_gpus']} GPUs (find_unused_parameters=True)"
                    )
                else:
                    logger.info(f"Single GPU detected: {self.gpus['num_gpus']} GPU")
        else:
            trainer_config.update({"accelerator": "cpu", "devices": 1})
    ...
    def evaluate_model(self, model: Challenge1Model, val_loader) -> Dict[str, float]:
        ...
        all_predictions = {"response_time": [], "hit_miss": []}

        all_targets = {"response_time": [], "hit_miss": []}
            ...
            for batch in val_loader:
                ...
                if torch.cuda.is_available():
                    ...
                    if isinstance(input_features, dict):
                        input_features = {
                            k: v.cuda() for k, v in input_features.items()
                        }
                    else:
                ...
                for key in all_predictions.keys():
                    if key in predictions:
                        if key == "hit_miss":
                            ...
                            all_predictions[key].extend(
                                predictions[key].squeeze().cpu().numpy()
                            )
                            all_targets[key].extend(
                                targets[key].squeeze().cpu().numpy()
                            )
        ...
        if all_targets["response_time"]:
            ...
            if np.any(np.isnan(rt_predictions)) or np.any(np.isinf(rt_predictions)):
                logger.warning(
                    f"Invalid predictions detected: NaN={np.any(np.isnan(rt_predictions))}, Inf={np.any(np.isinf(rt_predictions))}"
                )
                rt_predictions = np.where(
                    np.isfinite(rt_predictions), rt_predictions, np.median(rt_targets)
                )

            if np.any(np.isnan(rt_targets)) or np.any(np.isinf(rt_targets)):
                logger.warning(
                    f"Invalid targets detected: NaN={np.any(np.isnan(rt_targets))}, Inf={np.any(np.isinf(rt_targets))}"
                )
                rt_targets = np.where(
                    np.isfinite(rt_targets), rt_targets, np.median(rt_targets)
                )
        ...
        if all_targets["hit_miss"]:
            hm_acc = accuracy_score(
                all_targets["hit_miss"], all_predictions["hit_miss"]
            )
    ...
    def train(self):
        ...
        if trainer.global_rank == 0:
            ...
            logger.info(
                "Official Task: Predicting CCD behavioral outcomes from primary CCD EEG input."
            )
            logger.info("=" * 70)
        ...
        use_demographics = self.config.get("challenge1", {}).get(
            "use_demographics", False
        )
        use_sus_eeg = self.config.get("challenge1", {}).get("use_sus_eeg", False)

        logger.info(
            f"Feature Flags: use_demographics={use_demographics}, use_sus_eeg={use_sus_eeg}"
        )
        ...
        if "response_time_r2" in test_metrics:
            ...
            logger.info(
                f"   Response Time MAE: {test_metrics['response_time_mae']:.2f} ms"
            )
            logger.info(
                f"   Response Time RMSE: {test_metrics['response_time_rmse']:.2f} ms"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        # default="HBN-EEG\src\configs\challenge1_config.yaml",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode with reduced data"
    )
    parser.add_argument(
        "--compare",
        action="store_true",
    ...
    if args.compare or config.get("comparison", {}).get(
        "run_both_architectures", False
    ):
        ...
        for encoder, metrics in results.items():
            ...
            logger.info(
                f"  Hit/Miss Accuracy: {metrics.get('hit_miss_accuracy', 0):.4f}"
            )
