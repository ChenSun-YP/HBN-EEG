...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("challenge1_dataset.log"), logging.StreamHandler()],
)
...
class Challenge1Dataset(Dataset):
    ...
    def __init__(
        ...
        if self.channel_count != 128:
            logger.warning(
                f"Channel count should be 128 for EEG Foundation Challenge, got {self.channel_count}"
            )
        ...
        if samples is not None:
            ...
            logger.info(
                f"Reusing {len(self.samples)} pre-processed samples for {split} split"
            )
            return
        ...
        if cache_enabled:
            ...
            if self._load_from_cache(cache_file):
                logger.info(
                    f"Loaded {len(self.samples)} samples from cache for {split} split"
                )
                return

        # Parallel processing configuration
        self.use_parallel = config.get("parallel", {}).get("enabled", True)
        self.max_workers = config.get("parallel", {}).get(
            "max_workers", multiprocessing.cpu_count()
        )
        self.batch_size = config.get("parallel", {}).get(
            "batch_size", 100
        ...
        if self.use_parallel:
            logger.info(
                f"Using {self.max_workers} workers with batch size {self.batch_size}"
            )
    ...
    def _setup_bids_layout(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(release_dirs)} release directories: {[Path(d).name for d in release_dirs]}"
            )
            ...
            for release_dir in release_dirs:
                try:
                    ...
                    logger.info(
                        f"✅ {Path(release_dir).name}: {len(subjects)} subjects"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to initialize layout for {release_dir}: {e}"
                    )

            logger.info(
                f"BIDS layouts initialized successfully: {len(self.layouts)} releases, {total_subjects} total subjects"
            )
    ...
    def _load_metadata_and_qc(self) -> Tuple[Dict[str, pd.DataFrame], pd.DataFrame]:
        ...
        if self.releases is not None:
            ...
            logger.info(
                f"Loading metadata for all available releases: {releases_to_load}"
            )
        ...
        for release_name in releases_to_load:
            ...
            if participants_file.exists():
                ...
                logger.info(
                    f"Loaded metadata for {release_name}: {len(df)} participants"
                )
            else:
            ...
            if code_dir.exists():
                ...
                for qc_file in qc_files:
                    try:
                        ...
                        logger.info(
                            f"Loaded QC data from {qc_file.name}: {len(qc_df)} entries"
                        )
                    except Exception as e:
        ...
        if qc_dataframes:
            ...
            logger.info(
                f"QC whitelist created: {len(qc_whitelist)} valid recordings from {len(qc_combined)} total"
            )
        else:
            qc_whitelist = pd.DataFrame()
            logger.warning(
                "No QC files found, proceeding without quality control filtering"
            )
    ...
    def _create_samples(self):
        ...
        quick_test_enabled = quick_test.get("enabled", False)
        max_subjects = (
            quick_test.get("max_subjects", None) if quick_test_enabled else None
        )
    ...
    def _create_samples_with_bids(self):
        ...
        logger.info(
            f"Using BIDS layouts for file discovery for {self.split} split releases"
        )
        ...
        if self.releases is not None:
            ...
            logger.info(
                f"Processing specified releases for {self.split} split: {releases_to_process}"
            )
        else:
            # Fallback: process all available releases
            releases_to_process = [
                Path(release_dir).name for release_dir in self.layouts.keys()
            ]
            logger.info(
                f"Processing all available releases for {self.split} split: {releases_to_process}"
            )
        ...
        for release_dir, layout in self.layouts.items():
            ...
            if release_name not in releases_to_process:
                logger.debug(
                    f"Skipping {release_name} (not needed for {self.split} split)"
                )
                continue
        ...
        logger.info(
            f"Found {len(ccd_files)} total CCD files and {len(sus_files)} total SuS files for {self.split} split"
        )
        ...
        for file_path in ccd_files + sus_files:
            ...
            try:
                ...
                filename = path.name
                subject_id = (
                    filename.split("_")[0] if filename.startswith("sub-") else "unknown"
                )
                ...
                if "_task-" in filename:
                    ...
                    task = (
                        filename[task_start:task_end]
                        if task_end != -1
                        else filename[task_start:]
                    )
                ...
                if "_run-" in filename:
                    ...
                    run = (
                        filename[run_start:run_end]
                        if run_end != -1
                        else filename[run_start:]
                    )
        ...
        for subject_id, files in subject_files.items():
            ...
            if files["ccd_files"]:
                try:
                    ...
                    logger.warning(
                        f"Failed to extract release from {files['ccd_files'][0]}: {e}"
                    )
                    release = "unknown"
        ...
        if self.config.get("quick_test", {}).get("enabled", False):
            ...
            if len(all_subjects) > max_subjects:
                ...
                self.samples = self.samples[
                    : max_subjects * 50
                ]  # Approximate samples per subject
                logger.info(
                    f"Quick test mode: reduced samples from {total_samples_before} to {len(self.samples)}"
                )
    ...
    def _process_subjects_parallel(self, all_subjects: List[Dict]):
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            chunk_size = max(
                1, len(all_subjects) // (self.max_workers * 4)
            )  # Dynamic chunking
        ...
        for i, subject in enumerate(all_subjects):
            try:
                subject_samples = self._process_single_subject(
                    ...
                    self.resample_freq,
                    self.epoch_duration,
                    self.use_demographics,
                    self.use_sus_eeg,
                )
                if subject_samples:
                    self.samples.extend(subject_samples)
                    logger.info(
                        f"Created {len(subject_samples)} samples for {subject['subject_id']}"
                    )
            except Exception as e:
    ...
    def _process_single_subject(
        ...
        try:
            ...
            # extract subject demographic & psych data
            try:
            ...
            for ccd_file in ccd_files:
                ...
                try:
                    ...
                    ccd_eeg_data = Challenge1Dataset._load_ccd_eeg_data(
                        ccd_file, filter_params, resample_freq
                    )
    ...
    @staticmethod
    def _load_ccd_eeg_data(
        ccd_file: str, filter_params: Dict, resample_freq: int
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for ch in range(data.shape[0]):
                ...
                if np.std(ch_data) > 1e-8:
                    normalized_data[ch, :] = (ch_data - np.mean(ch_data)) / np.std(
                        ch_data
                    )
                else:
    ...
    def _load_sus_eeg_data(
        ...
        try:
            ...
            for sus_file in sus_files:
                ...
                if np.isnan(normalized_data).any():
                    logger.warning(
                        f"NaN detected in normalized SuS EEG data from {sus_file}"
                    )
                    continue
                if np.isinf(normalized_data).any():
                    logger.warning(
                        f"Inf detected in normalized SuS EEG data from {sus_file}"
                    )
                    continue
    ...
    def _load_ccd_trials(ccd_file: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            events_file = Path(ccd_file).parent / Path(ccd_file).name.replace(
                "_eeg.set", "_events.tsv"
            )
            ...
            for _, row in events_df.iterrows():
                ...
                # Detect button press
                    ...
                    if current_trial["contrast_change_time"] is not None:
                        current_trial["response_time"] = (
                            onset_time - current_trial["contrast_change_time"]
                        )
    ...
    @staticmethod
    def _extract_ccd_epoch(
        eeg_data: np.ndarray, trial: Dict, epoch_duration: float, sampling_rate: int
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if epoch.shape[0] != 128:
                logger.warning(
                    f"Expected 128 channels, got {epoch.shape[0]} for trial {trial['trial_id']}"
                )
                return None
            ...
            logger.warning(
                f"Error extracting CCD epoch for trial {trial['trial_id']}: {e}"
            )
            return None
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, Any], Dict[str, torch.Tensor]]:
        ...
        if idx >= len(self.samples):
            raise IndexError(
                f"Sample index {idx} out of bounds for dataset with {len(self.samples)} samples"
            )
    ...
    def _get_cache_file_path(self) -> Path:
        ...
        cache_file = (
            cache_dir
            / f"challenge1_samples_{self.split}_{features_str}_{config_hash}.pkl"
        )
        return cache_file
...
def create_challenge1_dataloaders(
    ...
    if not _BIDS_LAYOUTS_CACHE:
        ...
        logger.info(
            f"Shared BIDS layouts initialized: {len(_BIDS_LAYOUTS_CACHE)} releases, {total_subjects} total subjects"
        )
    else:
...
class EEGFoundationDataset(Dataset):
    ...
    def __init__(
        ...
        self.releases = [
            d.name
            for d in self.data_dir.iterdir()
            if d.is_dir() and "cmi_bids" in d.name
        ]
    ...
    def _create_samples(self):
        ...
        for release, metadata_df in self.metadata.items():
            ...
            for _, participant in metadata_df.iterrows():
                ...
                if self.subject_split:
                    if not any(
                        subject_id in split for split in self.subject_split.values()
                    ):
                        continue
        ...
        logger.info(
            f"Created {len(self.samples)} samples across {len(self.task_filter)} tasks"
        )
...
if __name__ == "__main__":
    ...
    config = {
        "data": {"channel_count": 129, "sampling_rate": 256},
        "preprocessing": {
            "filter_low": 0.5,
            "filter_high": 70.0,
            "resample_freq": 256,
        ...
        "parallel": {"enabled": True, "max_workers": 4, "batch_size": 10},
    }
