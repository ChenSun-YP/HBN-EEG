...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_len: int = 512, n_channels: int = 128):
        ...
        position_spatial = torch.arange(0, n_channels, dtype=torch.float).unsqueeze(
            1
        )  # (n_channels, 1)
        div_term_spatial = torch.exp(
            torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)
        )  # (d_model/2)
        pe_spatial[:, 0::2] = torch.sin(
            position_spatial * div_term_spatial
        )  # (n_channels, d_model/2)
        pe_spatial[:, 1::2] = torch.cos(
            position_spatial * div_term_spatial
        )  # (n_channels, d_model/2)
        self.register_buffer("pe_spatial", pe_spatial)

    def forward(
        self, x_patches: torch.Tensor, x_embedded: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if n_patches > self.pe_temporal.shape[0]:
            raise ValueError(
                f"n_patches ({n_patches}) exceeds max_len ({self.pe_temporal.shape[0]})"
            )
        if n_channels > self.pe_spatial.shape[0]:
            raise ValueError(
                f"n_channels ({n_channels}) exceeds spatial max ({self.pe_spatial.shape[0]})"
            )
        ...
        pe_spatial_expanded = (
            pe_spatial.unsqueeze(0).unsqueeze(0).expand(batch_size, n_patches, -1, -1)
        )
        ...
        combined_pe = pe_temporal_expanded + pe_spatial_expanded.mean(
            dim=2
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.pos_encoding = PositionalEncoding(
            d_model=d_model, max_len=self.n_patches, n_channels=n_channels
        )
...
class CNNEncoder(nn.Module):
    ...
    def __init__(
        self, n_channels: int = 128, n_times: int = 256, hidden_dim: int = 512
    ):
        """
...
class Challenge1Model(pl.LightningModule):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.response_time_scale = config.get("training", {}).get(
            "response_time_scale", 1000.0
        )
    ...
    def forward(
        self, input_features: Union[torch.Tensor, Dict[str, torch.Tensor]]
        ...
        # Validate input
        if torch.isnan(sus_eeg).any():
            raise ValueError("NaN detected in input EEG data!")
        if torch.isinf(sus_eeg).any():
        ...
        final_features = (
            pooled_features  # Use pooled features directly if no demographics
        )
    ...
    def training_step(
        ...
        if torch.isnan(rt_pred).any() or torch.isinf(rt_pred).any():
            logger.warning(
                "Invalid response time predictions detected in training step"
            )
        ...
        total_loss = sum(
            self.loss_weights[task] * loss for task, loss in losses.items()
        )
        ...
        for task, loss in losses.items():
            self.log(
                f"train/{task}_loss", loss, on_step=True, on_epoch=True, prog_bar=False
            )
    ...
    def validation_step(
        ...
        if torch.isnan(rt_pred).any() or torch.isinf(rt_pred).any():
            logger.warning(
                "Invalid response time predictions detected in validation step"
            )
            rt_pred = torch.clamp(rt_pred, 0.1, 2000.0)
        ...
        # Hit/miss loss (classification)
        losses["hit_miss"] = self.ce_loss(
            predictions["hit_miss"], targets["hit_miss"].squeeze()
        )
        ...
        if np.all(np.isfinite(rt_targets)) and np.all(np.isfinite(rt_predictions)):
            ...
            metrics["response_time_mae"] = mean_absolute_error(
                rt_targets, rt_predictions
            )
            metrics["response_time_rmse"] = np.sqrt(
                mean_squared_error(rt_targets, rt_predictions)
            )
        else:
        ...
        for task, loss in losses.items():
            self.log(
                f"val/{task}_loss", loss, on_step=False, on_epoch=True, prog_bar=False
            )
        ...
        return {"val_loss": total_loss, "val_metrics": metrics}
    ...
    def test_step(
        ...
        losses["response_time"] = self.mse_loss(
            predictions["response_time"].squeeze(), targets["response_time"].squeeze()
        )
        ...
        for task, loss in losses.items():
            self.log(
                f"test/{task}_loss", loss, on_step=False, on_epoch=True, prog_bar=False
            )
        ...
        self.log(
            "test/total_loss", total_loss, on_step=False, on_epoch=True, prog_bar=True
        )

        return {"test_loss": total_loss, "test_metrics": metrics}
...
def main():
    ...
    for encoder_type in ["cnn", "transformer"]:
        ...
        with torch.no_grad():
            ...
            print(
                f"   • Output shapes: {[f'{k}:{v.shape}' for k, v in predictions.items()]}"
            )
